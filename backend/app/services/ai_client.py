...
# 按上游域名控制兼容参数；部分 OpenAI-compatible 网关遇到不支持的可选参数时
# 不会返回明确的 400，而是延迟到生成阶段才抛 5xx。
...
class AIClient:
    ...
    async def _call_api_with_key_failover(
        ...
        for switch_index in range(MAX_API_CONFIG_SWITCHES + 1):
            try:
                # 终止类 Key 错误后强制刷新，下一轮才能选到刚提升为 active 的数据库通道。
            ...
            try:
                ...
                # 只有账号或 Key 终止类错误才在这里立即切换 Key。
                # 临时 5xx 交给 worker 重试，避免跨上游重复生成和重复扣费。
                last_error = exc
    ...
    async def _post_and_read_response(
        ...
        # 手动流式读取，拆分记录“等待响应头”和“接收响应体”的耗时，
        # 这是定位大 base64 响应慢传输的关键依据。
    ...
    async def _read_response_body_with_timeout(
        ...
        # 客户端库会在迭代响应体时抛出“响应体不完整”协议错误；
        # 单独 reader 任务可以保留已接收字节数，用于日志和任务审计。
    ...
    def _apply_provider_payload_options(payload: dict, api_config: ActiveApiConfig | None = None) -> None:
        if api_config is None:
            return
        # 可选上游参数集中在这里做兼容控制，避免供应商差异污染表单、计费和历史语义。
    ...
    def _handle_response(
        ...
        if response.status_code != 200:
            ...
            if response.status_code == 429:
                ...
                raise AIRateLimitError(safe_detail, retry_after)
            ...
            if 500 <= response.status_code <= 599:
                # 5xx 仍可能由 worker 重试，但先保留脱敏后的上游元数据，
                # 方便审计失败尝试，同时避免泄露 API Token。
